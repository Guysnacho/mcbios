    ...
    case "POST":
      try {
        ...
        const couponResponse = await stripe.coupons.create({
          ...
          applies_to: {
            products: [
              process.env.EB_CONF_REGISTRATION_STUDENT!,
              process.env.EB_CONF_REGISTRATION_POSTDOC!,
              process.env.EB_CONF_REGISTRATION_PROFESSIONAL!,
              process.env.CONF_REGISTRATION_STUDENT!,
              process.env.CONF_REGISTRATION_POSTDOC!,
              process.env.CONF_REGISTRATION_PROFESSIONAL!]
          }
        })
      ...
      try {
        ...
        if (session.status === "complete") {
          ...
          if (session!.metadata!.userId) {
            ...
            console.log(
              `Table update complete | user_role=${session!.metadata!.tier
              } | user_id=${session!.metadata!.userId} | member_only=${session!.metadata!.memberOnly
              }`
            );
